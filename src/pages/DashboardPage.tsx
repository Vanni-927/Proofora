...
import { Shield, Upload, Search, FileText, Eye, LogOut, TrendingUp } from "lucide-react";
		...
		<div className="min-h-screen bg-slate-950 text-white">
			...
			<div className="container mx-auto px-6 py-12 relative">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
					{[
						{ label: "Designs Protected", value: 247, icon: <Shield className="w-6 h-6" />, bgColor: "bg-purple-500/10", textColor: "text-purple-400" },
						{ label: "Scans Completed", value: 1543, icon: <Search className="w-6 h-6" />, bgColor: "bg-pink-500/10", textColor: "text-pink-400" },
						{ label: "Threats Detected", value: 12, icon: <TrendingUp className="w-6 h-6" />, bgColor: "bg-cyan-500/10", textColor: "text-cyan-400" },
					].map((stat, i) => (
						<motion.div
							key={i}
						...
						>
							<div className="flex items-center justify-between mb-4">
								<div className={`p-3 rounded-xl ${stat.bgColor} ${stat.textColor} group-hover:scale-110 transition-transform duration-300`}>
									{stat.icon}
								</div>
							...
							<div className="p-6">
								...
								<div className="space-y-2">
									<div className="flex items-center justify-between text-sm">
										...
										<span className="font-bold text-green-400">{design.similarity}%</span>
									</div>
